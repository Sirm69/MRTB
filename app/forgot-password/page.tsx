      ...
      <main className="flex-grow flex flex-col items-center pt-8 pb-12 px-6 bg-[#FCFDFF] relative overflow-hidden">
          ...
          <div className="mt-8">
            {!isSent ? (
              <button 
                type="submit" 
                disabled={loading}
                className="group flex items-center justify-center gap-2 bg-[#71b600] hover:bg-[#609a00] text-white font-black text-[11px] uppercase tracking-wider py-4 px-12 rounded-full transition-all shadow-md active:scale-95 disabled:opacity-70 min-w-[220px]"
              >
                {loading ? (
                  <>
              ...
              <button 
                ...
                className="group flex items-center justify-center gap-2 bg-[#066936] hover:bg-[#05542b] text-white font-black text-[11px] uppercase tracking-wider py-4 px-12 rounded-full transition-all shadow-md active:scale-95 min-w-[220px]"
              >
                Return to Login
                <ArrowRight size={14} style={{ color: accentYellow }} className="group-hover:translate-x-1 transition-transform" />
              </button>
        ...
        {message && (
          <div className={`mt-6 flex items-start justify-center gap-2 px-4 py-3 rounded-xl text-center text-xs font-semibold max-w-sm transition-all duration-300 ${
            messageType === "success"
              ? "bg-[#EEF6DF] text-[#066936] border border-[#65A30D]/40 shadow-sm"
              : "bg-red-50 text-red-600 border border-red-200"
          }`}>
            {messageType === "error" && <XCircle size={16} className="text-red-500 shrink-0 mt-0.5" />}
            {messageType === "success" && <CheckCircle size={16} className="text-[#066936] shrink-0 mt-0.5" />}
            <div>{message}</div>
          </div>
        )}
        ...
        <p className="mt-8 text-slate-500 text-xs font-medium relative z-10">
          ...
          <Link href="/auth/login" className="text-[#066936] font-bold hover:underline">
            Sign In Here
          </Link>
        </p>
      </main>
