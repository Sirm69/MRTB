...
export function AudiologyClinicalStep1({ visitationDate, fieldTeamMembers, onComplete, inspectionReport, isReviewMode, isReadOnly, onBack}: Step1Props) {
  ...
  const [clinicalStaff, setClinicalStaff] = useState({
    ...
    chiefAudiologists: "", seniorAudiologists: "", audiologists: "",
    internAudiologists: "", locumAudiologist: "", nyscAudiologist: "",
    totalAudiologist: "", comments: "", recommendations: ""
  });
  ...
  const [specialties, setSpecialties] = useState<Record<string, string>>({
    ...
    "Cochlear implant and implantable Devices": "",
    "Occupational Audiology": "",
    "Tinnitus and Hyperacusis Management": "",
    "Public Health, Research and Academic Audiology": ""
  });
  ...
  const [spaceEval, setSpaceForm] = useState({
    ...
    otherOfficesPrincipal: "", otherOfficesSenior: "", otherOfficesAudiologist: "", otherOfficesInterns: "", 
  ...
  const [equipEval, setEquipEval] = useState({
    rehabilitation: "", screening: "", diagnostics: "", infectiousControl: "", safetyMeasures: "", consumables: "",
    clinicalAudiology: "", paediatricAudiology: "", geriatricAudiology: "", rehabAudiology: "", educationalAudiology: "",
    vestibular: "", cochlearImplant: "", occupational: "", tinnitus: "", publicHealth: "",
    ...
    <form onSubmit={handleFormSubmission} className="space-y-6 w-full max-w-5xl mx-auto text-xs text-gray-800 antialiased font-medium pb-12">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1.5 pt-1">
            {[
              ...
              { label: "Total number of Chief Audiologists", f: "chiefAudiologists" },
              { label: "Total number of Senior Audiologists", f: "seniorAudiologists" },
              { label: "Total number of Audiologists", f: "audiologists" },
              { label: "Total number of Intern Audiologists", f: "internAudiologists" },
              { label: "Total number of Locum Audiologist", f: "locumAudiologist" },
              { label: "Total number of NYSC Audiologist", f: "nyscAudiologist" },
              { label: "Total number of Audiologists", f: "totalAudiologist" }
            ].map(item => (
            ...
            <div className="p-3 border border-gray-200 bg-slate-50/50 rounded-xl space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 font-semibold text-gray-700">
                {[
                  ...
                  { label: "Number of offices for Chief Audiologist", f: "otherOfficesChief" },
                  { label: "Number of offices for Principal Audiologist", f: "otherOfficesPrincipal" },
                  { label: "Number of offices for Senior Audiologist", f: "otherOfficesSenior" },
                  { label: "Number of offices for Audiologist", f: "otherOfficesAudiologist" },
                  { label: "Number of offices for Interns", f: "otherOfficesInterns" },
                  { label: "Number of offices for Locum/Corp members Audiologist", f: "otherOfficesLocumCorp" },
      ...
      <div className="bg-white p-5 rounded-xl border border-gray-300 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 font-semibold text-gray-700">
          ...
          <div>
            <label className="font-bold text-gray-600 block mb-1">Audiology (inpatient side wards)</label>
