...
export function OccupationalTherapyAcademicFormStep2({ visitationDate, fieldTeamMembers, institutionName, onComplete, inspectionReport, isReviewMode, isReadOnly, onBack}: Step2Props) {
  ...
  const [staffing, setStaffing] = useState({
    ...
    comments: "", recommendations: "", sponsoredMSc: "", sponsoredPhD: "", otherDegreesCount: "", otherDegreesSpecify: "", attendedCPD: ""
  });
  ...
  const [facilities, setFacilities] = useState({
    ...
    // OT Therapy Area
    adultTherapyAvailable: false, adultTherapyUnavailable: false,
    paediatricTherapyAvailable: false, paediatricTherapyUnavailable: false,
    therapyAreaSmall: false, therapyAreaLarge: false, therapyAreaCrowded: false, therapyAreaSpacious: false,
    therapyAreaFloorWooden: false, therapyAreaFloorTiled: false, therapyAreaFloorRugged: false, therapyAreaFloorConcrete: false, therapyAreaFloorSynthetic: false,
    therapyAreaAccessLarge: false, therapyAreaAccessSmall: false, therapyAreaAccessDisability: false,
    therapyAreaEquipAdequate: false, therapyAreaEquipInadequate: false,
    therapyAreaScreensAdequate: false, therapyAreaScreensInadequate: false,
    therapyAreaVentGood: false, therapyAreaVentFair: false, therapyAreaVentPoor: false,
    therapyAreaIllumGood: false, therapyAreaIllumFair: false, therapyAreaIllumPoor: false,
    therapyAreaComments: "", therapyAreaRecs: "",
    
    // OT Therapy Pool (Optional)
    poolAvailable: false, poolUnavailable: false,
    poolSizeSmall: false, poolSizeMedium: false, poolSizeLarge: false,
    poolFloorWooden: false, poolFloorTiled: false, poolFloorRugged: false, poolFloorConcrete: false, poolFloorSynthetic: false,
    poolLifeJacketsAvailable: false, poolLifeJacketsNotAvailable: false,
    poolEquipAdequate: false, poolEquipInadequate: false,
    poolMaintGood: false, poolMaintFair: false, poolMaintPoor: false,
    poolComments: "", poolRecs: "",
    ...
    toiletsStaffAdequate: false, toiletsStaffInadequate: false, toiletsStaffClean: false, toiletsStaffUnclean: false,
    toiletsStaffGenderYes: false, toiletsStaffGenderNo: false,
    toiletsStudentAdequate: false, toiletsStudentInadequate: false, toiletsStudentClean: false, toiletsStudentUnclean: false,
    toiletsStudentGenderYes: false, toiletsStudentGenderNo: false,
  ...
  const [clinical, setClinical] = useState({
    ...
    deptOtTherapistsAdequate: false, deptOtTherapistsInadequate: false, deptSpecialtiesList: "",
    ...
    <form onSubmit={handleFormSubmission} className="space-y-6 w-full max-w-5xl mx-auto text-xs text-gray-800 antialiased font-medium pb-24">
      ...
      <div className="bg-white p-5 rounded-xl border border-gray-300 shadow-sm text-center">
        <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider">
          INTERIM REPORT FOR ACADEMIC ACCREDITATION EXERCISE OF OCCUPATIONAL THERAPY PROGRAMME FOR THE MEDICAL REHABILITATION BOARD
        </h2>
      </div>
      ...
      <div className="bg-white p-5 rounded-xl border border-gray-300 shadow-sm space-y-4">
        ...
        <div className="p-4 bg-[#F5F8F2] border border-[#CDE1B4] rounded-xl space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-semibold text-gray-700">
            ...
            {/* OT Therapy Area */}
            <div className="bg-white p-3 border border-gray-200 rounded-lg space-y-3">
              <span className="font-bold text-gray-900 block">Therapy Area</span>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                <div>
                  <span className="font-bold text-gray-600 block">Adult Area:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.adultTherapyAvailable} onChange={(e) => setFacilities(p => ({ ...p, adultTherapyAvailable: e.target.checked, adultTherapyUnavailable: false }))} className="rounded" /> Available</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.adultTherapyUnavailable} onChange={(e) => setFacilities(p => ({ ...p, adultTherapyUnavailable: e.target.checked, adultTherapyAvailable: false }))} className="rounded" /> Unavailable</label>
                </div>
                <div>
                  <span className="font-bold text-gray-600 block">Paediatric Area:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.paediatricTherapyAvailable} onChange={(e) => setFacilities(p => ({ ...p, paediatricTherapyAvailable: e.target.checked, paediatricTherapyUnavailable: false }))} className="rounded" /> Available</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.paediatricTherapyUnavailable} onChange={(e) => setFacilities(p => ({ ...p, paediatricTherapyUnavailable: e.target.checked, paediatricTherapyAvailable: false }))} className="rounded" /> Unavailable</label>
                </div>
                <div>
                  <span className="font-bold text-gray-600 block">Size:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaSmall} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaSmall: e.target.checked }))} className="rounded" /> Small</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaLarge} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaLarge: e.target.checked }))} className="rounded" /> Large</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaCrowded} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaCrowded: e.target.checked }))} className="rounded" /> Crowded</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaSpacious} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaSpacious: e.target.checked }))} className="rounded" /> Spacious</label>
                </div>
                <div>
                  <span className="font-bold text-gray-600 block">Floor Structure:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaFloorWooden} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaFloorWooden: e.target.checked }))} className="rounded" /> Wooden</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaFloorTiled} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaFloorTiled: e.target.checked }))} className="rounded" /> Tiled</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaFloorRugged} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaFloorRugged: e.target.checked }))} className="rounded" /> Rugged</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaFloorConcrete} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaFloorConcrete: e.target.checked }))} className="rounded" /> Concrete</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaFloorSynthetic} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaFloorSynthetic: e.target.checked }))} className="rounded" /> Synthetic</label>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs pt-1 border-t border-gray-100">
                <div>
                  <span className="font-bold text-gray-600 block">Accessibility:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaAccessLarge} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaAccessLarge: e.target.checked }))} className="rounded" /> Large</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaAccessSmall} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaAccessSmall: e.target.checked }))} className="rounded" /> Small</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaAccessDisability} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaAccessDisability: e.target.checked }))} className="rounded" /> Disability Compliant</label>
                </div>
                <div>
                  <span className="font-bold text-gray-600 block">Equipment:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaEquipAdequate} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaEquipAdequate: e.target.checked, therapyAreaEquipInadequate: false }))} className="rounded" /> Adequate</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaEquipInadequate} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaEquipInadequate: e.target.checked, therapyAreaEquipAdequate: false }))} className="rounded" /> Inadequate</label>
                </div>
                <div>
                  <span className="font-bold text-gray-600 block">Screens:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaScreensAdequate} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaScreensAdequate: e.target.checked, therapyAreaScreensInadequate: false }))} className="rounded" /> Adequate</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.therapyAreaScreensInadequate} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaScreensInadequate: e.target.checked, therapyAreaScreensAdequate: false }))} className="rounded" /> Inadequate</label>
                </div>
                <div>
                  <span className="font-bold text-gray-600 block">Vent / Illum:</span>
                  <div className="flex gap-2">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 font-bold">Ventilation</span>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.therapyAreaVentGood} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaVentGood: e.target.checked, therapyAreaVentFair: false, therapyAreaVentPoor: false }))} className="scale-90" /> G</label>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.therapyAreaVentFair} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaVentFair: e.target.checked, therapyAreaVentGood: false, therapyAreaVentPoor: false }))} className="scale-90" /> F</label>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.therapyAreaVentPoor} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaVentPoor: e.target.checked, therapyAreaVentGood: false, therapyAreaVentFair: false }))} className="scale-90" /> P</label>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 font-bold">Illumination</span>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.therapyAreaIllumGood} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaIllumGood: e.target.checked, therapyAreaIllumFair: false, therapyAreaIllumPoor: false }))} className="scale-90" /> G</label>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.therapyAreaIllumFair} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaIllumFair: e.target.checked, therapyAreaIllumGood: false, therapyAreaIllumPoor: false }))} className="scale-90" /> F</label>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.therapyAreaIllumPoor} onChange={(e) => setFacilities(p => ({ ...p, therapyAreaIllumPoor: e.target.checked, therapyAreaIllumGood: false, therapyAreaIllumFair: false }))} className="scale-90" /> P</label>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-gray-100 font-bold text-gray-500">
                <div><label className="block text-[10px]">Comments</label><textarea value={facilities.therapyAreaComments} onChange={(e) => handleFieldUpdate(setFacilities, 'therapyAreaComments', e.target.value)} className="w-full mt-0.5 p-1 border border-gray-300 rounded h-10 resize-none outline-none text-[11px] font-semibold text-gray-900" /></div>
                <div><label className="block text-[10px]">Recommendations</label><textarea value={facilities.therapyAreaRecs} onChange={(e) => handleFieldUpdate(setFacilities, 'therapyAreaRecs', e.target.value)} className="w-full mt-0.5 p-1 border border-gray-300 rounded h-10 resize-none outline-none text-[11px] font-semibold text-gray-900" /></div>
              </div>
            </div>

            {/* OT Therapy Pool (Optional) */}
            <div className="bg-white p-3 border border-gray-200 rounded-lg space-y-3">
              <span className="font-bold text-gray-900 block">Therapy Pool (Optional)</span>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                <div>
                  <span className="font-bold text-gray-600 block">Pool Status:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolAvailable} onChange={(e) => setFacilities(p => ({ ...p, poolAvailable: e.target.checked, poolUnavailable: false }))} className="rounded" /> Available</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolUnavailable} onChange={(e) => setFacilities(p => ({ ...p, poolUnavailable: e.target.checked, poolAvailable: false }))} className="rounded" /> Unavailable</label>
                </div>
                <div>
                  <span className="font-bold text-gray-600 block">Pool Size:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolSizeSmall} onChange={(e) => setFacilities(p => ({ ...p, poolSizeSmall: e.target.checked, poolSizeMedium: false, poolSizeLarge: false }))} className="rounded" /> Small</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolSizeMedium} onChange={(e) => setFacilities(p => ({ ...p, poolSizeMedium: e.target.checked, poolSizeSmall: false, poolSizeLarge: false }))} className="rounded" /> Medium</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolSizeLarge} onChange={(e) => setFacilities(p => ({ ...p, poolSizeLarge: e.target.checked, poolSizeSmall: false, poolSizeMedium: false }))} className="rounded" /> Large</label>
                </div>
                <div>
                  <span className="font-bold text-gray-600 block">Floor Structure:</span>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolFloorWooden} onChange={(e) => setFacilities(p => ({ ...p, poolFloorWooden: e.target.checked }))} className="rounded" /> Wooden</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolFloorTiled} onChange={(e) => setFacilities(p => ({ ...p, poolFloorTiled: e.target.checked }))} className="rounded" /> Tiled</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolFloorRugged} onChange={(e) => setFacilities(p => ({ ...p, poolFloorRugged: e.target.checked }))} className="rounded" /> Rugged</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolFloorConcrete} onChange={(e) => setFacilities(p => ({ ...p, poolFloorConcrete: e.target.checked }))} className="rounded" /> Concrete</label>
                  <label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={facilities.poolFloorSynthetic} onChange={(e) => setFacilities(p => ({ ...p, poolFloorSynthetic: e.target.checked }))} className="rounded" /> Synthetic</label>
                </div>
                <div>
                  <span className="font-bold text-gray-600 block">Details & Maint:</span>
                  <div className="flex gap-2">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 font-bold">Life Jackets</span>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.poolLifeJacketsAvailable} onChange={(e) => setFacilities(p => ({ ...p, poolLifeJacketsAvailable: e.target.checked, poolLifeJacketsNotAvailable: false }))} className="scale-90" /> Yes</label>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.poolLifeJacketsNotAvailable} onChange={(e) => setFacilities(p => ({ ...p, poolLifeJacketsNotAvailable: e.target.checked, poolLifeJacketsAvailable: false }))} className="scale-90" /> No</label>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 font-bold">Maintenance</span>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.poolMaintGood} onChange={(e) => setFacilities(p => ({ ...p, poolMaintGood: e.target.checked, poolMaintFair: false, poolMaintPoor: false }))} className="scale-90" /> G</label>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.poolMaintFair} onChange={(e) => setFacilities(p => ({ ...p, poolMaintFair: e.target.checked, poolMaintGood: false, poolMaintPoor: false }))} className="scale-90" /> F</label>
                      <label className="flex items-center gap-0.5"><input type="checkbox" checked={facilities.poolMaintPoor} onChange={(e) => setFacilities(p => ({ ...p, poolMaintPoor: e.target.checked, poolMaintGood: false, poolMaintFair: false }))} className="scale-90" /> P</label>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-gray-100 font-bold text-gray-500">
                <div><label className="block text-[10px]">Comments</label><textarea value={facilities.poolComments} onChange={(e) => handleFieldUpdate(setFacilities, 'poolComments', e.target.value)} className="w-full mt-0.5 p-1 border border-gray-300 rounded h-10 resize-none outline-none text-[11px] font-semibold text-gray-900" /></div>
                <div><label className="block text-[10px]">Recommendations</label><textarea value={facilities.poolRecs} onChange={(e) => handleFieldUpdate(setFacilities, 'poolRecs', e.target.value)} className="w-full mt-0.5 p-1 border border-gray-300 rounded h-10 resize-none outline-none text-[11px] font-semibold text-gray-900" /></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white p-5 rounded-xl border border-gray-300 shadow-sm space-y-4">
        ...
        <div className="border-t border-gray-100 pt-3 space-y-3">
          <span className="font-bold text-gray-900 block text-xs">Occupational Therapy Department Ecosystem</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2 pt-2 text-gray-700 font-bold">
            {[
              { label: "Number of Occupational Therapists", ok: "deptOtTherapistsAdequate", bad: "deptOtTherapistsInadequate" },
              ...
              { label: "Availability of Teaching Aids", ok: "deptAidsAvailable", bad: "deptAidsNotAvailable", toggleOnly: true },
              { label: "Call Room Available", ok: "deptCallRoomAvailable", bad: "deptCallRoomNotAvailable", toggleOnly: true },
              { label: "Gender Sensitivity", ok: "deptGenderSensitiveYes", bad: "deptGenderSensitiveNo", toggleOnly: true },
              { label: "Furnishing Adequate", ok: "deptFurnishingAdequate", bad: "deptFurnishingInadequate", toggleOnly: true },
              { label: "Convenience Attachment", ok: "deptConvenienceAttachedYes", bad: "deptConvenienceAttachedNo", toggleOnly: true }
            ].map((row, idx) => (
