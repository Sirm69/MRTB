...
function CalendarContent() {
  ...
  return (
    <>
      {/* Page Title */}
      <div className="mb-4">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800 tracking-tight">Inspection Calendar</h1>
        <p className="text-sm text-gray-500">Track acceptances, rescheduling request lists, and active visit audits</p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
              ...
              {/* Day Headers */}
              <div className="grid grid-cols-7 gap-px text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
            ...
            {selectedDate && (
              <div className="bg-white rounded-3xl p-6 shadow-sm border border-[#5D9C0E]/30 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <h3 className="text-sm font-bold text-gray-800 mb-4 flex items-center gap-2">
                  <CalendarDays size={18} className="text-[#5D9C0E]" /> 
                ...
                <div className="space-y-3">
                  {selectedInspections.map((item, idx) => (
                    <div key={idx} className="p-4 bg-gray-50 rounded-2xl border border-gray-100 flex flex-col sm:flex-row justify-between sm:items-center gap-4 text-xs">
                      <div className="space-y-1">
                        <p className="font-bold text-gray-850 text-sm leading-tight">{item.name}</p>
                        <p className="text-gray-400 text-[10px]">{item.profession} • {item.category}</p>
                      </div>
                      <div className="flex items-center gap-3">
                        {item.visitation_accepted ? (
                          <span className="text-[#5D9C0E] bg-[#EEF6DF] font-bold px-3 py-1 rounded-full flex items-center gap-1 text-[10px] border border-[#CDE1B4]/40">
                            <CheckCircle size={12} /> Accepted
                          </span>
                        ) : item.is_rescheduled ? (
                          <span className="text-amber-600 bg-amber-50 font-bold px-3 py-1 rounded-full flex items-center gap-1 text-[10px]">
                            <Clock size={12} /> Rescheduled
                          </span>
                        ) : (
                          <span className="text-blue-500 bg-blue-50 font-bold px-3 py-1 rounded-full flex items-center gap-1 text-[10px]">
                            <Clock size={12} /> Awaiting Acceptance
                          </span>
                        )}
                        <button 
                          onClick={() => router.push(`/admin/dashboard`)} 
                          className="bg-[#5D9C0E] hover:bg-[#4a7c0b] text-white font-bold px-4 py-1.5 rounded-full text-[10px] transition-colors shadow-sm cursor-pointer"
                        >
                          View Case
                        </button>
                      </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              <h3 className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2">
                <Calendar size={18} className="text-[#5D9C0E]" /> Upcoming Visits ({inspections.length})
              </h3>
              ...
              {inspections.length > 0 ? (
                  ...
                  {inspections.map((item, idx) => (
                    <div key={idx} className="border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                      ...
                      <div className="flex justify-between items-center gap-2 text-[10px]">
                        <span className="text-gray-400 leading-none truncate max-w-[120px]">{item.profession}</span>
                        {item.visitation_accepted ? (
                          <span className="text-[#5D9C0E] font-bold flex items-center gap-0.5"><CheckCircle size={10} /> Confirmed</span>
                        ) : (
                          <span className="text-amber-500 font-bold flex items-center gap-0.5"><Clock size={10} /> Pending</span>
                        )}
                      </div>
                    </div>
                  ))}
                ...
                <p className="text-xs text-gray-400 italic">No inspections scheduled yet.</p>
              )}
            </div>

            <div className="bg-[#5D9C0E] text-white rounded-3xl p-6 shadow-sm flex gap-3 text-xs leading-relaxed">
              <ShieldAlert size={20} className="shrink-0 mt-0.5" />
              <div>
                <span className="font-bold block mb-1">Administrative Note:</span>
                Inspection dates confirmed by facilities are dispatched automatically to designated regional coordinators and field officers.
              </div>
            </div>
          </div>
